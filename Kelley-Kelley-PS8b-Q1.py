...
def QuickSelect(A, p, r, split):
    ...
    if p == r:
        ...
        # call partition with a duplicate of A
        # use a duplicate of A because we don't want partition to
        # change A since this is the finding of the value
        # not the sorting of the array
        q = Partition(A, p, r)
...
def QuickSort(A, p, r):
    ...
    if (p < r):
        ...
        if r - p >= 4:
            # gets the range of r to p and divides by 4 to get how many
            # indices to right of p it has to be
            split = (r - p + 1) // 4
            # adds p since need from p not from beginning
            split += p
