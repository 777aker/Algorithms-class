...
def draw_graph(G, kruskal_selected_edges, sorted_edges):
    ...
    if len(kruskal_selected_edges) > 0:
        ...
        nx.draw_networkx_edges(G, pos,
                               edgelist=kruskal_selected_edges, width=2.5, edge_color='r')
...
def union(a, b):
    # storing variables for use later
    leaderA = find(a)
    leaderB = find(b)
    sizeA = 0
    sizeB = 0
    # get the size of a and b
    for x in components[leaderA]:
        sizeA += 1
    for x in components[leaderB]:
        sizeB += 1
    # if a is greater than b
    if sizeA > sizeB:
        # go through everything in b
        for x in components[leaderB]:
            # add everything in b to a
            components[leaderA].append(x)
            # change the leader of everything in b to a
            leader_dict[x] = leaderA
        # get rid of the component b since its now all in a
        del components[leaderB]
    # do the same thing except switch a and b for comments from line 108-116
    elif sizeB > sizeA:
        for x in components[leaderA]:
            components[leaderB].append(x)
            leader_dict[x] = leaderB
        del components[leaderA]
    # do same thing above except if they are equal randomly select which
    # one to put in which
    else:
...
if __name__ == '__main__':
    ...
    # make k = 0 to run Kruskal's normally
    # to get values of k 2...10 just change k and run
    k = 10
    # this goes through the edges in ascending order
    for u, v in sorted_edges:
        # this breaks k iterations before the end
        if 127 - k == 0:
            break
        # if they are not in the same connected component call union and append them to kruskal selected
        if find(u) != find(v):
            union(u, v)
            kruskal_selected_edges.append((u,v))
            k += 1

    # this keeps track of the disconnected components whose minimum weight to the others have been solved for
    solvedComponents = {}
    # this goes through the edges in ascending order
    for u, v in sorted_edges:
        # if two vertices are not in the same connected component
        if find(u) != find(v):
            # if they are not a value in solvedComponents, ie they haven't been connected yet
            if not ((find(u), find(v)) in solvedComponents or (find(v), find(u)) in solvedComponents):
                # put them in solved and put their value as the length between them and the cities being connected
                solvedComponents.update({(find(u), find(v)) : [u, v, length_of_edge[(u, v)]]})
    # print out the results from above
    for x in solvedComponents:
        print(x, solvedComponents[x])
